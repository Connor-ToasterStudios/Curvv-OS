...
switch_to_lm:
    ...
    ; Mapping data to the PT
    ; Note that eax was already set to the desired location in the above code
    mov eax, PML4T_OFFSET + 0x3000
    mov ebx, 3
    mov ecx, 512 ; Set the loop count to 512
    switch_to_pm_memmap_loop:
        mov [eax], ebx
        add ebx, 0x1000
        add eax, 8
        loop switch_to_pm_memmap_loop
    ; Set PAE and PGE
    mov eax, cr4
    or eax, 1 << 5 | 1 << 7
    mov cr4, eax
